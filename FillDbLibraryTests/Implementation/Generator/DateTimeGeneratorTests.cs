...
namespace FillDbLibraryTests.Implementation.Generator
{
  public class DateTimeGeneratorTests
  {
    ...
    [InlineData(999, 0)]
    [InlineData(11, 0)]
    public void ConcructorWithWrongPrecisionFail(int precision, int maxLength)
    {
      // Arrange & Act & assert
      Assert.Throws<ArgumentException>(() => new DateTimeGenerator(Mock.Of<IRandomNumber>(), precision, maxLength));
    }
